...
<body>
	...
	<main class="container">
		...
		<c:if test="${user.isAdmin()}">
			<div class="mb-3">
				<a href="/turimosTierraMedia/users/create.do" class="btn btn-primary"
				...
				<c:forEach items="${users}" var="tmp_user">
					<tr>
						...
						<td><c:if test="${user.admin && (!tmp_user.admin || tmp_user.id == user.id)}">
								<a href="/turimosTierraMedia/users/edit.do?id=${tmp_user.id}"
									class="btn btn-light rounded-0" role="button"><i
									class="bi bi-pencil-fill"></i></a>
								<a href="/turimosTierraMedia/users/delete.do?id=${tmp_user.id}"
